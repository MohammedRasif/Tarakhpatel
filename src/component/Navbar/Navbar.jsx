  ...
  return (
    <div className="text-white fixed top-0 left-0 w-full z-50 flex justify-center mt-3 md:mt-5 px-3 md:px-0">
      ...
      >
        ...
        <NavLink to="/">
          <div>
            <img src={image || "/placeholder.svg"} className="h-9 md:h-12 w-auto" alt="Logo" />
          </div>
        </NavLink>
        ...
        <div className="hidden lg:flex items-center space-x-10 text-gray-300 text-[16px]">
          <NavLink
            to="/vision"
            className={({ isActive }) =>
              `relative font-medium transition-all duration-300 hover:text-white pb-2 ${isActive ? "text-white" : ""}`
            }
          >
            {({ isActive }) => (
              <>
        ...
        <div className="hidden lg:block bg-gradient-to-r from-[#00953B] to-[#0068F0] rounded-full px-6 py-3 border border-white/20 shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-105 cursor-pointer">
          <button className="font-bold text-white text-[16px] tracking-wide">Join the Waitlist</button>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-20 left-3 right-3 bg-black/95 backdrop-blur-xl rounded-3xl border border-gray-700 shadow-2xl overflow-hidden">
          <div className="flex flex-col p-6 space-y-4">
            <NavLink
              ...
              className={({ isActive }) =>
                `text-gray-300 font-medium text-lg py-3 px-4 rounded-xl transition-all duration-300 ${
                  isActive ? "bg-gradient-to-r from-[#00953B] to-[#0068F0] text-white" : "hover:bg-white/10"
                }`
              }
            >
              Vision
            ...
            {/* Mobile CTA Button */}
            <div className="pt-2">
              <button className="w-full bg-gradient-to-r from-[#00953B] to-[#0068F0] rounded-xl py-4 px-6 font-bold text-white text-base tracking-wide shadow-lg hover:shadow-xl transition-all duration-300">
                Join the Waitlist
              </button>
            </div>
          </div>
